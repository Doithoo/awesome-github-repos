...
function createOwner(repository) {
  ...
  const profileUrl = getSafeUrl(owner.profileUrl, { githubOnly: true });
  ...
  const avatarUrl = getSafeUrl(owner.avatarUrl);
...
export function createRepositoryCard(repository, { now = new Date() } = {}) {
  ...
  const fullName = typeof repository.fullName === 'string'
    ? repository.fullName
    : repository.name ?? '';
  const repositoryUrl = getSafeUrl(repository.repositoryUrl, { githubOnly: true });
  ...
  const homepage = getSafeUrl(repository.homepage);
  if (homepage) {
