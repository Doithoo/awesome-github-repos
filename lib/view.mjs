import { formatNumber, formatRelativeDate } from './catalog.mjs';
...
function safeHttpsUrl(value, { githubOnly = false } = {}) {
...
function secureExternalLink(className, url, label) {
  const link = element('a', {
    className,
    attributes: { href: url, 'aria-label': label },
  });
...
function createOwner(repository) {
  ...
  const profileUrl = safeHttpsUrl(owner.profileUrl, { githubOnly: true });
  const ownerElement = profileUrl
    ? secureExternalLink('repository-owner', profileUrl, `View ${login} on GitHub`)
    : element('div', { className: 'repository-owner' });
  const avatarUrl = safeHttpsUrl(owner.avatarUrl);
  ...
  if (avatarUrl) {
    ...
    ownerElement.append(avatar);
  } else {
    ownerElement.append(element('span', {
...
export function createRepositoryCard(repository, { now = new Date() } = {}) {
  ...
  const fullName = typeof repository.fullName === 'string'
    ? repository.fullName
    : repository.name ?? '';
  const repositoryUrl = safeHttpsUrl(repository.repositoryUrl, { githubOnly: true });

  identity.append(createOwner(repository));
  const title = repositoryUrl
    ? secureExternalLink('repository-name', repositoryUrl, `View ${fullName} on GitHub`)
  ...
  if (homepage) {
    const homepageLink = secureExternalLink(
      'repository-homepage',
      homepage,
      `Visit the ${fullName} homepage`,
    );
...
export function renderError(panel, message) {
  const status = element('div', { className: 'status-error', attributes: { role: 'alert' } });
...
export function renderEmpty(panel) {
  const status = element('div', { className: 'status-empty', attributes: { role: 'status' } });
